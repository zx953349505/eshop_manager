...
<body>
				...
				<div class="block">
					...
					<div id="page-filter" class="block-body collapse in">
						<div class="search-well">
							<form  class="form-inline" role="search" action="${pageContext.request.contextPath }/permission/addPer" method="post">
								<input type="text" class="form-control" placeholder="请输入权限名称" name="permission_name" /> 
								<button>
							...
							<table class="table">
								...
								<tbody>
									<c:forEach items="${requestScope.perList }" var="perInfo">
										<tr>
											<td>${perInfo.permission_id }</td>
											<td>${perInfo.permission_name }</td>
											<td>
												<a href="#" title="编辑"><i class="icon-pencil"></i></a>
	              								<a href="javascript:deleteClick('${perInfo.permission_id }')" title="删除"><i class="icon-remove"></i></a>
											</td>
										</tr>
									</c:forEach>
	...
	function deleteClick(permission_id) {
		if (confirm("你确定删除吗？")) {
			show_loading();
			var url="${pageContext.request.contextPath }/permission/deletePer";
			var data={
					permission_id:permission_id
			};
			
			$.post(url,data,function(response){
				var responseUrl="${pageContext.request.contextPath }/permission/perPage";
				if(response=="200"){
