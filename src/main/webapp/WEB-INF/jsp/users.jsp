				...
				<div class="block">
					...
					<div id="page-filter" class="block-body collapse in">
						<div class="search-well">
							<form  class="form-inline" role="search" action="${pageContext.request.contextPath }/user/selectAllUserList?" method="post">
			...
			<div class="row-fluid">
						...
						<c:set var="firstPage">
							...
							<c:if test="${requestScope.currentPage!=1 }">
								${pageContext.request.contextPath }/user/selectAllUserList?currentPage=1${selectCondition }
							</c:if>
						</c:set>
						<c:set var="lastPage">
							<c:if test="${requestScope.currentPage==1 }">#</c:if>
							<c:if test="${requestScope.currentPage!=1 }">
								${pageContext.request.contextPath }/user/selectAllUserList?currentPage=${requestScope.currentPage-1 }${selectCondition }
							</c:if>
						</c:set>
						<c:set var="nextPage">
						...
						<c:set var="endPage">
							<c:if test="${requestScope.currentPage==requestScope.totalPage }">#</c:if>
							<c:if test="${requestScope.currentPage!=requestScope.totalPage }">
								${pageContext.request.contextPath }/user/selectAllUserList?currentPage=${requestScope.totalPage }${selectCondition }
							</c:if>
						</c:set>
							...
							<ul>
								...
								<c:forEach begin="${(group-1)*10+1 }" end="${group*10 }" step="1" var="i">
									<c:set var="middlePage">
										${pageContext.request.contextPath }/user/selectAllUserList?currentPage=${i }${selectCondition }
									</c:set>
