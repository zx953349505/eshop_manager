...
<html lang="en">
  ...
  <body class=""> 
		...
		<div class="container-fluid">
					...
					<div id="page-filter" class="block-body collapse in">
						<div class="search-well">
							<form class="form-inline" role="search"
								action="${pageContext.request.contextPath }/book/bookManagerPage" method="post">
			...
			<div class="row-fluid">
					...
					<div class="search-well">
						<form class="form-inline" action="#">
							<select name="control">
								...
								<option value="garbage">移入回收站</option>
					...
					<table class="table">
						...
						<tbody>
							...
							<tr>
								...
								<td>
								<a href="http://127.0.0.1/eshop/book/bookInfo.action?book_id=${bookInfo.book_id }"title="查看" target="_blank">
								<i class="icon-search"></i>
								</a>
								<a href="${pageContext.request.contextPath }/book/updateBook/${bookInfo.book_id }" title="编辑">
								<i class="icon-pencil"></i>
								</a> 
								<a href="javascript:deleteBook('${pageContext.request.contextPath }', ${bookInfo.book_id })" title="删除">
				...
				<c:set var="firstPage">
					...
					<c:if test="${requestScope.currentPage!=1 }">
						${pageContext.request.contextPath }/book/bookManagerPage?currentPage=1${selectCondition }
					</c:if>
				</c:set>
				<c:set var="lastPage">
					<c:if test="${requestScope.currentPage==1 }">
					...
					<c:if test="${requestScope.currentPage!=1 }">
						${pageContext.request.contextPath }/book/bookManagerPage?currentPage=${requestScope.currentPage-1 }${selectCondition }
					</c:if>
				</c:set>
				<c:set var="nextPage">
				...
				<c:set var="endPage">
					...
					<c:if test="${requestScope.currentPage!=requestScope.totalPage }">
						${pageContext.request.contextPath }/book/bookManagerPage?currentPage=${requestScope.totalPage }${selectCondition }
					</c:if>
				</c:set>
					<ul>
						...
						<c:set var="middlePage">
								${pageContext.request.contextPath }/book/bookManagerPage?currentPage=${i }${selectCondition }
						</c:set>
	...
	<script src="${pageContext.request.contextPath }/static/lib/book.js"></script>
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			updateBookStatus('${pageContext.request.contextPath }');
